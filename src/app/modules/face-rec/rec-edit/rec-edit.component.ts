...
export class RecEditComponent implements OnInit {
  ...
  ngOnInit() {
    ...
    this.type = this.currentData.Type;
    this.sex = this.currentData.Sex;
    this.account = this.currentData.User ? this.currentData.User.Uid : '';
    this.phone = this.currentData.Telephone;
    this.org = this.currentData.Organization ? this.currentData.Organization.Name : '';
    this.wechat = this.currentData.Wechat;
    this.imgUrl = this.currentData.Img ? this.currentData.Img : 'assets/images/no_image.png';
    this.getAccounts();
  }
  ...
  save() {
    ...
    this.faceService.editData(data)
      .subscribe(val => {
        Swal(
          '保存成功！',
          '',
