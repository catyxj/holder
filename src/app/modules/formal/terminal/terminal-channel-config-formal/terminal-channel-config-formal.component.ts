...
import {AlarmRuleComponent} from '../modals/alarm-rule/alarm-rule.component';
import {ActivatedRoute} from '@angular/router';

import Swal from 'sweetalert2';
import {RangeConfigComponent} from '../modals/range-config/range-config.component';

@Component({
  selector: 'app-terminal-channel-config-formal',
  templateUrl: './terminal-channel-config-formal.component.html',
  styleUrls: ['./terminal-channel-config-formal.component.css']
})
export class TerminalChannelConfigFormalComponent implements OnInit {
  public uid;
  public code;
  public name;
  public basic;
  ...
  save() {
    ...
    for (let i = 0; i < this.analogueList.length; i++) {
      if (this.analogueList[i].Parameter.Name) {
        ...
        if (this.analogueList[i].slaveAddress > 255 || this.analogueList[i].slaveAddress < 1) {
          this.nzmodalService.error({
            nzTitle: '通道配置更新失败',
    ...
    for (let i = 0; i < this.switchList.length; i++) {
      if (this.switchList[i].Parameter.Name) {
        ...
        if (this.switchList[i].slaveAddress > 255 || this.switchList[i].slaveAddress < 1) {
          this.nzmodalService.error({
            nzTitle: '通道配置更新失败',
    ...
    for (let i = 0; i < this.rangeList.length; i++) {
      if (this.rangeList[i].Parameter.Name) {
        ...
        if (this.rangeList[i].slaveAddress > 255 || this.rangeList[i].slaveAddress < 1) {
          this.nzmodalService.error({
            nzTitle: '通道配置更新失败',
